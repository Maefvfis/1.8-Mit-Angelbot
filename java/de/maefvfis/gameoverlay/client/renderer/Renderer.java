...
public class Renderer {
	...
	public void onRender(RenderWorldLastEvent event) {
		if (this.minecraft != null && ConfigurationHandler.renderSpawns) {
			...
			if (player != null) {
				PlayerPosition.x = (float) (player.lastTickPosX + (player.posX - player.lastTickPosX) * event.partialTicks);
				PlayerPosition.y = (float) (player.lastTickPosY + (player.posY - player.lastTickPosY) * event.partialTicks);
				PlayerPosition.z = (float) (player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * event.partialTicks);
	...
	private void render() {
		...
		for (Vector4i blockPos : Events.SPAWN_LIST) {
			...
			delta = 0.0f;
			block = this.minecraft.theWorld.getChunkFromBlockCoords(new BlockPos(blockPos.x, blockPos.y, blockPos.z)).getBlock(new BlockPos(blockPos.x, blockPos.y, blockPos.z));
			if (block != null) {
				if (block == Blocks.snow || block == Blocks.wooden_pressure_plate || block == Blocks.stone_pressure_plate || block == Blocks.light_weighted_pressure_plate || block == Blocks.heavy_weighted_pressure_plate) {
					block.setBlockBoundsBasedOnState(this.minecraft.theWorld, new BlockPos(blockPos.x, blockPos.y, blockPos.z));
