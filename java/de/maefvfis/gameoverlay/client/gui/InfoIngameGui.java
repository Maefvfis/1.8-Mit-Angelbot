...
public class InfoIngameGui extends GuiScreen {
	...
	public InfoIngameGui() {
		...
		// Chunk / ChunkBiome
		fontRender.drawStringWithShadow("Chunk: " + Integer.valueOf(X >> 4) + " (" + Integer.valueOf(X & 15) + "), " + Integer.valueOf(Z >> 4) + " (" + Integer.valueOf(Z & 15) + ")" + " | " + chunk.getBiome(new BlockPos(X & 0xf, X & 0xf,0), mc.theWorld.getWorldChunkManager()).biomeName, 5, currentY, 0xffffff);
		currentY += 10;
		...
		if (mc.theWorld != null && mc.theWorld.isBlockLoaded(new BlockPos(X, Y, Z)))
        {
			...
			fontRender.drawStringWithShadow("Light: " + chunk.getLightFor(EnumSkyBlock.BLOCK, new BlockPos(X & 15, Y, Z & 15)) + " / " + chunk.getLightFor(EnumSkyBlock.SKY, new BlockPos(X & 15, Y, Z & 15)), 5,currentY, 0xffffff);
			currentY += 10;
        }
